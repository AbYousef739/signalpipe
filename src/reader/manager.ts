...
export const FEED_DELAY_MS = 5_000
...
export interface ReadCounts { stations: number; sent: number; entries: number; empty: number; skipped: number; errors: number }
...
export async function readOnce(deps: ReaderDeps = defaultDeps): Promise<ReadCounts> {
  ...
  const counts: ReadCounts = { stations: stations.length, sent: 0, entries: 0, empty: 0, skipped: 0, errors: 0 }
  if (!stations.length) {
  ...
  for (let i = 0; i < stations.length; i++) {
    ...
    try {
      const entries = parseFeed(await deps.fetchText(station.rss_url as string), MAX_ENTRIES)
      if (!entries.length) {
        counts.empty++
